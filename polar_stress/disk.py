...
def post_process_synthetic_data(
    principal_diff, theta_p, sigma_xx, sigma_yy, tau_xy, t_sample, C, lambda_light, outname
):
    ...
    plt.contour(X, Y, fringe_order, levels=integer_levels, colors="black", linewidths=1.0)
    ...
    plt.pcolormesh(
        ...
        sigma_yy_MPa,
        cmap="RdBu_r",
        norm=SymLogNorm(linthresh=sigma_yy_max / 1e3, vmin=-sigma_yy_max, vmax=sigma_yy_max),
    )
    ...
    # Summary statistics
    plt.subplot(4, 4, 16)
    plt.text(0.1, 0.8, f"Load: {P:.0f} N/m", fontsize=12, transform=plt.gca().transAxes)
    plt.text(
        0.1,
        0.7,
