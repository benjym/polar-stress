...
class TestFourStepPolarimetry:
    ...
    def test_simulate_four_step_polarimetry_different_wavelengths(self, test_parameters, sample_stress):
        ...
        for i, (wavelength, C) in enumerate(zip(test_parameters['wavelengths_nm'], test_parameters['C_values'])):
            I0_pol, I45_pol, I90_pol, I135_pol = simulate_four_step_polarimetry(
                sigma_xx, sigma_yy, sigma_xy, C, nu, L, wavelength, S_i_hat, I0
            )
            
            # Check that results vary with wavelength (due to different C values)
            assert np.all(np.isfinite([I0_pol, I45_pol, I90_pol, I135_pol])), f"All intensities should be finite for wavelength {wavelength}"
            assert np.all(np.array([I0_pol, I45_pol, I90_pol, I135_pol]) >= 0), f"All intensities should be non-negative for wavelength {wavelength}"
    ...
    def test_simulate_four_step_polarimetry_stress_dependence(self, test_parameters):
        ...
        for stress in stress_levels:
            I0_pol, I45_pol, I90_pol, I135_pol = simulate_four_step_polarimetry(
                stress, 0.0, 0.0,  # Uniaxial stress
                C, nu, L, wavelength, S_i_hat, I0
            )
...
class TestSyntheticDiskData:
    ...
    def test_synthetic_disk_data_basic(self, test_parameters):
        ...
        try:
            ...
            assert synthetic_images.shape == expected_image_shape, f"Expected shape {expected_image_shape}, got {synthetic_images.shape}"
            
            expected_stress_shape = (grid_size, grid_size, 3)  # 3 stress components
            assert stress_field.shape == expected_stress_shape, f"Expected stress shape {expected_stress_shape}, got {stress_field.shape}"
            
            expected_mask_shape = (grid_size, grid_size)
            assert mask.shape == expected_mask_shape, f"Expected mask shape {expected_mask_shape}, got {mask.shape}"
...
class TestDiskUnderCompression:
    ...
    def test_disk_under_diametral_compression_stress_distribution(self):
        ...
        try:
            ...
            stress_variation = np.std(stress_field[center_y-5:center_y+5, center_x-5:center_x+5, 0])
